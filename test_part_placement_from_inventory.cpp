...
int main(int argc, char** argv) {
    ...
    part_pose_wrt_cam=box_inspector_image_.models[1].pose; //MAJOR ASSUMPTIONS
    cam_pose=box_inspector_image_.pose;
    part_pose_wrt_world=compute_stPose(cam_pose,part_pose_wrt_cam);

    while(!boxInspector.compare_pose(place_part, part_pose_wrt_world)) {
      if(!robotBehaviorInterface.place_part_in_box_no_release(place_part)) {
        ROS_INFO("placement failed");
        break;
    }
